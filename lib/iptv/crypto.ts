...
/** AES-256-GCM under IPTV_CREDENTIALS_KEY, so playlist sources and Xtream logins are never stored in the clear. */
function key(): Buffer {
  const raw = process.env.IPTV_CREDENTIALS_KEY?.trim();
  if (!raw) throw new Error("IPTV_CREDENTIALS_KEY is not configured.");
